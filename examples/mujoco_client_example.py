...
while True:
    pos = 0.5 * np.sin(2 * np.pi * 0.01 * time.time())
    ...
    mobile_base_pos = [-1.0, -1.0, -np.pi/2]
    # mobile_base_pos = [0.0, -1.0, 0]
    # mobile_base_pos = [0.0, 0, -np.pi/2]
    # mobile_base_pos = [0.0, 0.0, -np.pi / 2] if pos > 0 else [0.0, -1.0, -np.pi / 2]
